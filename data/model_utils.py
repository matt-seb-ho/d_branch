...
class LM:
    def __init__(
        ...
        **model_args,
        ...
        if self.model_type == "vllm":
            ...
            raise ValueError(
                "Invalid model_type. Choose 'vllm', 'hf', 'openai', or 'anthropic'."
            )

    def generate(
        self, question, partial_answer, num_rollouts=None, single_pass_mode=False
    ):
    ...
    def generate_api(self, prompt: str, num_rollouts) -> List[str]:
        ...
        with ThreadPoolExecutor(max_workers=num_rollouts) as executor:
            futures = [
                executor.submit(send_request, prompt) for _ in range(num_rollouts)
            ]
